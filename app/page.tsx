...
export default function Page() {
  ...
  const [mode, setMode] = useState<"forecast" | "fragility">("forecast");
  ...
  return (
    <div className="container">
      {mode === "forecast" ? (
        <>
          <h1>🦢 블랙스완 가격 확률 시뮬레이터</h1>
          <p className="subtitle">
            종목을 고르고 시행 횟수를 정하면, <b>정규분포가 아닌 멱법칙(팻테일) + 블랙스완</b>을
            얹은 몬테카를로로 가격의 확률 분포·꼬리 위험·목표가 도달 확률을 계산합니다.
          </p>
        </>
      ) : (
        <>
      ...
      <div className="tabs">
        ...
        <button
          className={mode === "fragility" ? "active" : ""}
          onClick={() => setMode("fragility")}
        >
          🛡 전략 프래질리티
        </button>
      </div>
      ...
      <p className="subtitle" style={{ marginTop: 24 }}>
        ⚠️ 교육용 시뮬레이터입니다. 합성·과거 데이터 기반이며 투자 자문이 아닙니다.
      </p>
    </div>
  );
}
